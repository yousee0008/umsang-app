...
export default function Visualize() {
  ...
  const ranked = products
    .map((p,i)=>({ p, score: avgColor ? rgbDistance(avgColor, productColors[i] ?? hexToRgb('#777777')) : 0 }))
          ...
          {!roomURL ? (
            ...
            <div className="relative overflow-auto">
              ...
              <div className="absolute top-0 left-0 pointer-events-none"
                   style={{ transform:`translate(${dx}px,${dy}px) scale(${scale})`, transition:'transform 40ms linear' }}>
                <NextImage src={active.images[0]} alt={active.title} width={300} height={300}
                           className="pointer-events-auto border rounded-xl bg-white/70" />
              </div>
            </div>
          )}
        </div>
        <div className="space-y-4">
          <div className="card p-4">
            ...
            <ul className="space-y-2">
              {ranked.map((r, idx)=>(
                <li key={r.p.sku}>
                  <button className={`btn w-full text-left ${idx===selected?'bg-gray-100':''}`}
          ...
          <div className="card p-4 space-y-3">
            ...
            <label className="block text-sm">Scale: {scale.toFixed(2)}
              <input type="range" min="0.2" max="2.0" step="0.02" value={scale}
                     onChange={(e)=>setScale(parseFloat(e.target.value))} className="w-full" /></label>
            <label className="block text-sm">Move X: {dx}px
              <input type="range" min="-500" max="500" step="1" value={dx}
                     onChange={(e)=>setDx(parseInt(e.target.value))} className="w-full" /></label>
            <label className="block text-sm">Move Y: {dy}px
              <input type="range" min="-500" max="500" step="1" value={dy}
                     onChange={(e)=>setDy(parseInt(e.target.value))} className="w-full" /></label>
          </div>
        </div>
      </div>
    </div>
